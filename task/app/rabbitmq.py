...
from app.services.task_service import TaskService  # Импортируем сервис управления задачами
from app.repositories.task_repo import TaskRepo  # Импортируем репозиторий для задач
...
async def send_assignee_update_message(assignee_id: int):
    ...
    await channel.default_exchange.publish(
        ...
        routing_key='slusarchuk_assignee_update_queue'
    )
...
async def consume_tasks(loop: AbstractEventLoop) -> AbstractRobustConnection:
    ...
    task_created_queue = await channel.declare_queue('slusarchuk_task_created_queue', durable=True)
